...
def test_books(session: WebDriver):
    ...
    # login
    session.find_element(By.ID, "login").click()  # go to login page
    session.find_element(By.ID, "userName").send_keys(USER)
    session.find_element(By.ID, "password").send_keys(PASSWORD)
    session.find_element(By.ID, "login").click()  # go to bookstore page
    sleep(2)
