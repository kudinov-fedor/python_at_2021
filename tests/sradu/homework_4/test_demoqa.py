from tests.sradu.homework_4.utils import (Condition, wait_for_condition, wait_for_all_elements, wait_for_text_to_change,
...
def test_alert_confirm_box_ok(driver):
    ...
    click_by_action_chains(driver, menu_item)

    # 2 - перейти до Alerts
    menu_sub_item = get_menu_item(driver, SideMenuLocators.LIST_MENU_SUB_ITEM, "Alerts")
    click_by_action_chains(driver, menu_sub_item)

    # 3 - натиснути `Click me` праворуч від варіанту `On button click, confirm box will appear`
    click_button = AlertsLocators.get_click_btn_for_item(driver, "On button click, confirm box will appear")
    click_by_action_chains(driver, click_button)

    # 4 - перевірити, що alert містить дані: Do you confirm action?
    alert = wait_for_condition(driver, Condition.ALERT_PRESENT)
    ...
    confirm_text_element = wait_for_condition(driver, Condition.ELEMENT_VISIBLE, locator)
...
def test_droppable(driver):
    ...
    click_by_action_chains(driver, droppable)

    # 3 - перевірити, що початкова назва більшого блоку, в який пересуватимемо маленький блок, становить `Drop here`
    droppable_text_locator = DragAndDropLocators.get_locator_for_droppable_text_element("simpleDropContainer")
    droppable_text_element = wait_for_condition(driver, Condition.ELEMENT_VISIBLE, droppable_text_locator)
    ...
    source_element = wait_for_condition(driver, Condition.ELEMENT_VISIBLE, source_locator)
    target_element = wait_for_condition(driver, Condition.ELEMENT_VISIBLE, target_locator)
